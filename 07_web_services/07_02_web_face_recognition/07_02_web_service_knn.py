...
def detect_faces_in_image(file_stream):
    ...
    closest_distances = knn_clf.kneighbors(unknown_face_encodings, n_neighbors=2)
    print('closest_distances:{}'.format(closest_distances))
    # 目前的distance_threshold為0.4,要小於等於0.4才算是辨識成功，這個值可以自己調整。
    are_matches = [closest_distances[0][i][0] <= 0.4 for i in range(len(unknown_face_locations))]
    #看看結果為何
    print(are_matches)
    # 回傳結果,姓名，位置，結果
    [who_found.append(name) if rec.any() else ("未知人員", loc) for name, loc, rec in zip(knn_clf.predict(unknown_face_encodings), unknown_face_locations, are_matches)]
