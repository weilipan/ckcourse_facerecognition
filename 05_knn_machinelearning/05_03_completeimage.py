...
def show_prediction_labels_on_image(img_folder,img_path, predictions):
    """
    將預測的結果視覺化.
    :img_path: 用來預測的圖片路徑
    :predictions: 利用KNN預測完的結果
    """
    filename=join(img_folder, image_file)
    bgr_image = cv2.imread(filename)

    # BGR轉換為RGB，人臉辨識用
    unknown_image = cv2.cvtColor(bgr_image,cv2.COLOR_BGR2RGB)
    ...
    #將影像存檔
    resultfolder='result'
    cv2.imwrite(join(resultfolder,img_path),frame)
...
if __name__ == "__main__":
    ...
    for image_file in listdir(testdir):
        ...
        for name, (top, right, bottom, left) in predictions:
            ...
            # 將影像呈現出來
        show_prediction_labels_on_image(testdir, image_file, predictions)